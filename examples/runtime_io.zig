//! flecs' parallel pipeline running on a `std.Io` executor: per-sync-point task
//! threads are dispatched as async tasks on the executor's pool, and awaited.
...
pub fn main() !void {
    ...
    flecs.runtime(.{ .io = io }); // flecs task threads -> std.Io executor
    ...
    std.debug.print("{d}/{d} correct on the std.Io executor\n", .{ ok, n });
}
